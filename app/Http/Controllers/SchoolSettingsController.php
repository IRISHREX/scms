    ...
    public function id_card_store(Request $request)
    {
        ...
        if ($request->type == 'Student') {
            $settings = [
                ...
                'cards_per_page' => 'required|in:1,2,4,6,8',
                'student_id_card_fields' => 'nullable',
                'student_id_card_styles' => 'nullable',
            ...
            $settings = [
                ...
                'staff_cards_per_page' => 'required|in:1,2,4,6,8',
                'staff_id_card_fields' => 'nullable',
                'staff_id_card_styles' => 'nullable',
                'signature' => 'nullable|image|max:2048',
            ];
        }
        ...
        try {
            ...
            foreach ($settings as $key => $rule) {
                ...
                } else if ($key == 'student_id_card_styles') {
                    $styles = $request->input('student_id_card_styles', []);
                    $data[] = [
                        "name" => $key,
                        "data" => json_encode($styles),
                        "type" => "string"
                    ];
                } else if ($key == 'staff_id_card_styles') {
                    $styles = $request->input('staff_id_card_styles', []);
                    $data[] = [
                        "name" => $key,
                        "data" => json_encode($styles),
                        "type" => "string"
            ...
            // Student
            if ($request->type == 'Student') {
                // Reset all student form fields to not display on ID card
                $this->formField->builder()->where('user_type', 1)->update(['display_on_id' => 0]);
                // Set selected extra form fields to display on ID card
                if ($request->extra_form_fields) {
                    $this->formField->builder()->where('user_type', 1)->whereIn('id', $request->extra_form_fields)->update(['display_on_id' => 1]);
                }
            }
            
            // Staff
            if ($request->type == 'Staff') {
                // Reset all staff form fields to not display on ID card
                $this->formField->builder()->where('user_type', 2)->update(['display_on_id' => 0]);
                // Set selected extra form fields to display on ID card
                if ($request->extra_form_fields) {
                    $this->formField->builder()->where('user_type', 2)->whereIn('id', $request->extra_form_fields)->update(['display_on_id' => 1]);
                }
            }
