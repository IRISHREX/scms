...
class FeesController extends Controller
{
        ...
        if ($feesId) {
            ...
            foreach ($res as $row) {
                ...
                if ($row->fees_paid) {
                    ...
                    // Add discount amount if exists
                    $tempRow['discount_amount'] = $row->fees_paid->discount ?? 0;    
                } else {
                    $tempRow['paid_amount'] = 0;
                    $tempRow['discount_amount'] = 0;
                }
    ...
    public function payCompulsoryFeesStore(Request $request)
    {
        ...
        $request->validate([
            ...
            'installment_fees.*.discount' => 'nullable|numeric|min:0',
            'installment_fees.*.discount_type' => 'nullable|in:fixed,percentage',
            'installment_fees.*.payment_type' => 'nullable|in:full,partial',
            'installment_fees.*.partial_amount' => 'nullable|numeric|min:0',
            'mode'               => 'required|in:1,2,3',
            'cheque_no'          => 'required_if:mode,2',
            'upi_id'             => 'required_if:mode,3',
            'extra_fee'          => 'nullable|numeric|min:0',
            'extra_fee_name'     => 'nullable|string|max:255',
            'installment_extra_fee' => 'nullable|numeric|min:0',
            'installment_extra_fee_name' => 'nullable|string|max:255',
            ...
            'upi_id.required_if'           => 'UPI ID is required for UPI payments',
            'mode.in'                      => 'Payment mode must be Cash (1), Cheque (2), or UPI (3)'
        ]);
        ...
        try {
            ...
            if ($request->installment_mode) {
                if (!empty($request->installment_fees)) {
                    foreach ($request->installment_fees as $key => $installment) {
                        // Skip if installment doesn't have required data
                        if (!isset($installment['id'])) {
                            continue;
                        }
                        
                        // Check if amount key exists
                        if (!isset($installment['amount']) || empty($installment['amount'])) {
                            return response()->json([
                                'error' => true,
                                'message' => 'Invalid installment data. Please refresh the page and try again.',
                                'code' => 400
                            ], 400);
                        }
                        
                        // Handle partial payment
                        $paymentType = $installment['payment_type'] ?? 'full';
                        $partialAmount = $installment['partial_amount'] ?? 0;
                        
                        // Use partial amount if payment type is partial, otherwise use full amount
                        if ($paymentType === 'partial' && $partialAmount > 0) {
                            $installmentAmount = $partialAmount;
                        } else {
                            $installmentAmount = $installment['amount'];
                        }
                        
                        $discountValue = $installment['discount'] ?? 0;
                        $discountType = $installment['discount_type'] ?? 'fixed';
                        
                        $installmentDiscount = 0;
                        if ($discountValue > 0) {
                            if ($discountType === 'percentage') {
                                $installmentDiscount = ($installmentAmount * $discountValue) / 100;
                            } else {
                                $installmentDiscount = $discountValue;
                            }
                        }
                        
                        $amount += ($installmentAmount - $installmentDiscount);
                        $totalDiscount += $installmentDiscount;
                    }
                }
                $amount += $request->advance;
                $extraFee = $request->installment_extra_fee ?? 0;
                $extraFeeName = $request->installment_extra_fee_name ?? '';
            } else {
                $discountValue = $request->discount ?? 0;
                $discountType = $request->discount_type ?? 'fixed';
                
                $baseAmount = $request->enter_amount ?: $request->total_amount;
                ...
                $extraFeeName = $request->extra_fee_name ?? '';
            }
            
            // Add extra fee to total amount
            $amount += $extraFee;
            ...
            if (empty($feesPaid)) {
                $feesPaidResult = $this->feesPaid->create([
                    ...
                    'discount_type'       => $request->installment_mode ? 'mixed' : ($request->discount_type ?? 'fixed'),
                ]);
            } else {
            ...
            if ($request->installment_mode == 1) {
                if (!empty($request->installment_fees)) {
                    foreach ($request->installment_fees as $installment_fee) {
                        // Skip if installment doesn't have required data
                        if (!isset($installment_fee['id'])) {
                            continue;
                        }
                        
                        // Check if amount key exists
                        if (!isset($installment_fee['amount']) || empty($installment_fee['amount'])) {
                            DB::rollBack();
                            return response()->json([
                                'error' => true,
                                'message' => 'Invalid installment data. Please refresh the page and try again.',
                        ...
                        // Handle partial payment
                        $paymentType = $installment_fee['payment_type'] ?? 'full';
                        $partialAmount = $installment_fee['partial_amount'] ?? 0;
                        
                        // Use partial amount if payment type is partial, otherwise use full amount
                        if ($paymentType === 'partial' && $partialAmount > 0) {
                            $installmentAmount = $partialAmount;
                        } else {
                            $installmentAmount = $installment_fee['amount'];
                        }
                        
                        $discountValue = $installment_fee['discount'] ?? 0;
                        $discountType = $installment_fee['discount_type'] ?? 'fixed';
                        ...
                        $finalAmount = $installmentAmount - $installmentDiscount;
                        ...
                        $compulsoryFeeData = array(
                            ...
                            'mode'           => $request->mode,
                            'cheque_no'      => $request->mode == 2 ? $request->cheque_no : ($request->mode == 3 ? $request->upi_id : null),
                            'amount'         => $finalAmount,
            ...
            DB::commit();
            ResponseService::successResponse("Payment Completed Successfully");
        } catch (Throwable $e) {
            DB::rollback();
            ...
            if (str_contains($e->getMessage(), 'Undefined array key')) {
                $errorMessage = 'Invalid form data. Please refresh the page and try again.';
    ...
    public function optionalFees(Request $request)
    {
        ...
        $feesClassTypes = FeesClassType::where('optional', 1)
                        ->with(['fees_type' => function ($query) {
                            $query->select('id', 'name');
                        }])->get();

        return view('fees.optional-fees', compact('session_year_all', 'class_section', 'feesClassTypes'));
    }
            ...
            $sql = $this->user->builder()
                    ...
                    ->with([
                        'optional_fees' => function ($query) use ($filter_optional_fees) {
                            ...
                            ->with(['fees_class_type' => function ($query) {
