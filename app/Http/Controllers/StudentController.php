...
class StudentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'image' => 'nullable|mimes:jpeg,png,jpg,svg|image',
            'dob' => 'required',
        ...
        try {
            ...
            if (empty($request->guardian_email) || !filter_var($request->guardian_email, FILTER_VALIDATE_EMAIL)) {
                $guardianUser = $this->user->builder()->role('Guardian')->where('mobile', $request->guardian_mobile)->first();
                if ($guardianUser) {
                    $request->merge(['guardian_email' => $guardianUser->email]);
                } else {
                    $request->merge(['guardian_email' => 'guardian_' . $request->guardian_mobile . '@' . $_SERVER['HTTP_HOST']]);
                }
            }
    ...
    public function update($id, Request $request)
    {
        ...
        $rules = [
            ...
            'dob' => 'required',
            'session_year_id' => 'required|numeric',
            'guardian_email' => 'nullable|max:255|unique:users,email',
            'roll_number' => 'nullable|numeric',
        ];
        if (is_numeric($request->guardian_id)) {
            $rules['guardian_email'] = 'nullable|unique:users,email,' . $request->guardian_id;
        }
        ...
        try {
            ...
            $userService->updateStudentUser($id, $request->first_name, $request->last_name, $request->mobile, $request->dob, $request->gender, $request->image, $sessionYear->id, $request->extra_fields ?? [], $guardian->id, $request->current_address, $request->permanent_address, $request->reset_password, $request->class_section_id,$request->roll_number );
            DB::commit();
    ...
    public function rollNumberUpdate(Request $request)
    {
        ...
        $validator = Validator::make(
            $request->all(),
            ['roll_number_data' => 'required|array', 'roll_number_data.*.roll_number' => 'required'],
        ...
        try {
            ...
            foreach ($request->roll_number_data as $data) {
                // Validate that student_id and roll_number exist
                if (!isset($data['student_id']) || !isset($data['roll_number'])) {
                    throw new \Exception('Invalid roll number data format');
                }
                ...
                // Optional: Validate roll number uniqueness within the class section
                // Uncomment the code below if you need to check for duplicate roll numbers
                // $student = $this->student->find($data['student_id']);
                // $class_roll_number_data = $this->student->builder()
                //     ->where('class_section_id', $student->class_section_id)
                //     ->where('roll_number', $data['roll_number'])
                //     ->where('id', '!=', $data['student_id'])
                //     ->count();
                // if ($class_roll_number_data > 0) {
                //     throw new \Exception(trans('roll_number_already_exists'));
                // }
            ...
            DB::rollBack();
            ResponseService::logErrorResponse($e, "Student Controller -> rollNumberUpdate");
            ResponseService::errorResponse();
        }
    }
    ...
    public function rollNumberShow(Request $request)
    {
        ...
        try {
            ...
            foreach ($res as $row) {
                ...
                // Display old roll number and editable input for new roll number
                $tempRow['new_roll_number'] = "<input type='hidden' name='roll_number_data[" . $index . "][student_id]' class='form-control' readonly value='" . $row->student->id . "'> <input type='text' name='roll_number_data[" . $index . "][roll_number]' class='form-control form-control-sm' placeholder='Enter new roll number' value='" . ($row->student->roll_number ?? '') . "'>";
    ...
    public function generate_id_card(Request $request)
    {
        ...
        try {
            ...
            // Decode styles
            $settings['student_id_card_styles'] = json_decode($settings['student_id_card_styles'] ?? '[]', true);
            $settings['staff_id_card_styles'] = json_decode($settings['staff_id_card_styles'] ?? '[]', true);
            ...
            // Get cards per page setting
            $cardsPerPage = (int) ($settings['cards_per_page'] ?? 1);
            
            // Calculate paper size based on cards per page
            if ($cardsPerPage > 1) {
            ...
            $settings['page_height_px'] = ($settings['page_height'] * 3.7795275591) . 'px';
            $settings['page_width_px'] = ($settings['page_width'] * 3.7795275591) . 'px';
            $settings['cards_per_page'] = $cardsPerPage;
