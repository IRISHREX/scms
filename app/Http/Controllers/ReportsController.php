...
class ReportsController extends Controller
{
    ...
    public function exam_reports()
    {
        ...
        return view('reports.exam.exam-reports', compact('classes', 'subjects', 'mediums', 'sessionYears', 'exams', 'classSections', 'totalExamCount', 'reportCardTemplates'));
    }
    ...
    public function yearlyResultShow(Request $request)
    {
        ...
        foreach ($res as $row) {
            ...
            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            // Ensure roll_number is present at top-level for the table (fallback to related user->student if needed)
            $tempRow['roll_number'] = $row->roll_number ?? ($row->user->student->roll_number ?? '-');
    ...
    public function subjectWiseResultShow(Request $request)
    {
        ...
        foreach ($res as $row) {
            ...
            $tempRow = $row->toArray();
            $tempRow['id'] = $row->user_id; // Set ID to user_id for PDF route
            $tempRow['student_id'] = $row->user_id; // Add student_id for checkbox selection
            $tempRow['no'] = $no++;
            $tempRow['total_marks'] = number_format($totalMarks, 2);
            $tempRow['obtained_marks'] = number_format($obtainedMarks, 2);
            $tempRow['percentage'] = number_format($percentage, 2);
    ...
    // Get all student IDs for bulk print
    public function getAllStudentIdsForBulkPrint(Request $request)
    {
        $sessionYearId = request('session_year_id');
        $classSectionId = request('class_section_id');
        $examId = request('exam_id');
        ...
        $query = DB::table('exam_results')
            ->join('students', 'exam_results.student_id', '=', 'students.user_id')
        ...
        if ($examId) {
            $query->where('exam_results.exam_id', $examId);
        }

        $studentIds = $query->distinct()
            ->pluck('exam_results.student_id')
            ->toArray();
    ...
    public function rankWiseResultPdf($student_id)
    {
        try {
            ...
            // Use Report Card Template if available
            $reportCardTemplate = $this->reportCardTemplate->builder()->latest()->first();
    ...
    // Bulk Print - All students PDFs in one file
    public function bulkPrintAllStudentsPdf(Request $request)
    {
        try {
            $sessionYearId = $request->get('session_year_id');
            $classSectionId = $request->get('class_section_id');
            ...
            if (!$sessionYearId) {
                return redirect()->back()->with('error', 'Please select session year');
            }
            ...
            // Get exams - filter by selected exam_id if provided
            $examsQuery = $this->exam->builder()
                ->with([
                    'timetable' => function ($q) {
                        $q->with('exam_marks');
                    }
            ...
            foreach ($studentIds as $student_id) {
                $results = $this->examResult->builder()
                    ->with([
                        ...
                        'class_section.section',
                        'class_section.medium',
                        'user' => function ($q) use ($student_id, $examId) {
                            $q->with([
                                'student.guardian',
                                'exam_marks' => function ($q) use ($student_id, $examId) {
                                    $q->whereHas('timetable', function ($q) use ($student_id, $examId) {
                                        $q->where('student_id', $student_id);
                                        if ($examId) {
                                            $q->where('exam_id', $examId);
                                        }
            ...
            if (empty($allStudentsData)) {
                return redirect()->back()->with('error', 'No results found for selected students');
            }

            // Pass all data to view
            return view('reports.exam.bulk-rank-wise-exam-result-pdf', compact(
                'settings',
                'grades',
                'exams',
                'allStudentsData'
    ...
    // Bulk Download Selected PDFs
    public function bulkDownloadSelectedPdfs(Request $request)
    {
        try {
            ...
            $query = DB::table('exam_results')
                ...
                ->where('exam_results.session_year_id', $sessionYearId)
                ->where('exams.publish', 1)
                ->whereIn('exam_results.student_id', $studentIdArray);
            ...
            $validStudentIds = $query->distinct()
                ->pluck('exam_results.student_id')
                ->toArray();

            if (empty($validStudentIds)) {
                return redirect()->back()->with('error', 'No valid students found for selected criteria');
            }
    ...
    /**
     * Bulk Print Report Card - All students with selected template
     */
    public function bulkPrintReportCard(Request $request)
    {
        try {
            $sessionYearId = $request->get('session_year_id');
            $classSectionId = $request->get('class_section_id');
            ...
            if (!$templateId) {
                return redirect()->back()->with('error', 'Please select report card template');
            }

            // Get the selected report card template
            $reportCardTemplate = $this->reportCardTemplate->builder()->find($templateId);
            
            if (!$reportCardTemplate) {
                return redirect()->back()->with('error', 'Report card template not found');
            }
    ...
    /**
     * Print Selected Students Report Card with selected template
     */
    public function printSelectedReportCard(Request $request)
    {
        try {
            $studentIds = $request->get('student_ids', '');
            $sessionYearId = $request->get('session_year_id');
