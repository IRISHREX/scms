...
class PromoteStudentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $promoteDataArray = json_decode($request->promote_data, true);

            $passStudentsIds = [];
            $failStudentsIds = [];
            $leftStudentSIds = [];
            $passStudentsRollNumbers = [];

            foreach ($promoteDataArray as $key => $data) {
                // Get the actual student ID (not user_id)
                $studentRecord = $this->student->builder()->where('user_id', $data['student_id'])->first();
                ...
                if ($data['result'] == 1) {
                    ...
                    if ($data['status'] == 1) {
                        ...
                        $passStudentsRollNumbers[$data['student_id']] = $data['roll_number'] ?? null;
                    }
                } else {
                    // Student Failed
            ...
            if (!empty($passStudentsIds)) {
                ...
                foreach ($passStudentsIds as $userId) {
                    $student = $this->student->builder()->where('user_id', $userId)->first();

                    if ($student) {
                        $rollNumber = null;

                        // Check if roll number is provided and not empty
                        if (isset($passStudentsRollNumbers[$userId]) && !empty($passStudentsRollNumbers[$userId])) {
                            $rollNumber = (int) $passStudentsRollNumbers[$userId];
                        } else {
            ...
            if (!empty($promoteStudentData)) {
                $this->promoteStudent->upsert($promoteStudentData, ['class_section_id', 'student_id', 'session_year_id'], ['status', 'result']);
            }
