...
class TeacherApiController extends Controller
{
        ...
        try {
            if ($request->student_id) {
                ...
                if ($request->exam_id) {
                    ...
                    $sql = $this->user->builder()->select('id', 'first_name', 'last_name', 'image')->role('Student')->whereIn('id', $studentIds)->with([
                        'marks' => function ($query) use ($timetable) {
                            $query->where('exam_timetable_id', $timetable->id)
                                ->select('id', 'exam_timetable_id', 'student_id', 'marks', 'obtained_marks');
                        }
                    ])
            ...
            $data = $sql->map(function ($item) {
                if (isset($item->marks) && count($item->marks)) {
                    $item->marks = $item->marks->map(function ($m) {
                        $m->marks = json_decode($m->marks, true); // <-- JSON decode here
                        return $m;
                    });
                    return $item;
                }
    ...
    public function getExamList(Request $request)
    {
        ...
        try {
            ...
            $sql = $this->exam->builder()->with('session_year', 'class')->select('id', 'name', 'description', 'exam_type', 'class_id', 'start_date', 'end_date', 'session_year_id', 'publish')
                ->with([
            ...
            if ($request->class_section_id) {
                $sql = $sql->whereHas('class.class_sections', callback: function ($q) use ($request) {
            ...
            // FIXED: Move these queries outside the loop and handle null class_section_id
            $subjectTeacherIds = [];
            $classTeacher = null;
            ...
            if ($request->class_section_id) {
                ...
                $classTeacher = ClassTeacher::where('teacher_id', Auth::user()->id)
                    ->where('class_section_id', $request->class_section_id)
                    ->first();
            }
            ...
            foreach ($exam_data_db as $data) {
                ...
                if ($current_date == $data->start_date && $current_date == $data->end_date) {
                    ...
                    if ($current_date >= $data->start_date && $current_date <= $data->end_date) {
                        ...
                        $exam_status = "0"; // FIXED: Changed from "2" to "0" for upcoming
                    } else if ($current_date > $data->end_date) { // FIXED: Changed >= to >
                        $exam_status = "2"; // FIXED: Changed from "1" to "2" for completed
                    } else {
                        $exam_status = null;
                    }
                }
                ...
                if (count($data->timetable)) {
                    foreach ($data->timetable as $key => $timetable) {
                        // FIXED: If no class_section_id filter, show all timetables
                        // If class_section_id is provided, filter by teacher permissions
                        $shouldInclude = false;
                        ...
                        if (!$request->class_section_id) {
                            ...
                            // Class teacher sees everything in their class
                            $shouldInclude = true;
                        } elseif (in_array($timetable->class_subject->subject_id, $subjectTeacherIds)) {
                        ...
                        if ($shouldInclude) {
                            ...
                            $class_subject = [
                                ...
                                'class_id' => $timetable->class_subject->class_id,
                                'subject_id' => $timetable->class_subject->subject_id,
                                'subject' => $subject
                            ];
                            $timetable_data[] = [
                                'id' => $timetable->id,
                ...
                $examItem = array(
                    ...
                    'exam_type' => json_decode($data->exam_type ?? '[]', 1),
                );

                // FIXED: Simplified status filtering logic
                if (isset($request->status) && $request->status != 3) {
                    // Filter by exact status match
                    if ($request->status == $exam_status) {
                        $exam_data[] = $examItem;
                    }
                } else {
                    // No status filter or status=3 - include all
                    $exam_data[] = $examItem;
                }
            }
