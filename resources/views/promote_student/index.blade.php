                                        ...
                                        <form action="{{ route('promote-student.store') }}" data-success-function="formSuccessFunction" class="create-form mt-6 pt-3" id="formdata">
                                            ...
                                            <table aria-describedby="mydesc" class='table promote_student_table' id='promote_student_table_list'
                                                ...
                                                <tr>
                                                    ...
                                                    <th scope="col" data-field="roll_number" 
                                                        data-formatter="rollNumberInputFormatter">
                                                        Update Roll No.
                                                    </th>
                                                </tr>
                                                </thead>
...
@section('script')
        ...
        // Ye purana rollNumberInputFormatter hai - isme koi change nahi
        function rollNumberInputFormatter(value, row, index) {
            let oldValue = row.roll_number ?? '';
            let studentUserId = row.user?.id || row.user_id;

            return `<input type="number" 
                    class="form-control roll-input" 
                    name="roll_update[${studentUserId}]" 
                    value="${oldValue}"
                    style="width:120px">`;
        }

        // Ye updated submit handler hai - isme result aur status properly handle hote hain
        $('form').on('submit', function (e) {
            let promoteData = [];

            $('#promote_student_table_list').bootstrapTable('getData').forEach(function (row) {
                let studentUserId = row.user?.id || row.user_id;

                // Get roll number from input
                let rollNumber = $(`input[name="roll_update[${studentUserId}]"]`).val() || row.roll_number || '';

                // Get result and status from row data (table already has these values)
                let result = row.result !== undefined ? row.result : 1;
                let status = row.status !== undefined ? row.status : 1;
            ...
            // Remove any existing promote_data hidden input
            $('input[name="promote_data"]').remove();

            $('<input>').attr({
                type: 'hidden',
                name: 'promote_data',
                value: JSON.stringify(promoteData)
            }).appendTo(this);
        });
    </script>
@endsection
