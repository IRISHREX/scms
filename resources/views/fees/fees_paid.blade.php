                        ...
                        <table aria-describedby="mydesc" class='table' id='table_list' data-toggle="table"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" data-field="discount_amount" data-sortable="false" data-align="center" data-formatter="discountFormatter">{{ __('Discount') }}</th>
                                    ...
                                    <th scope="col" data-field="paid_amount" data-sortable="false" data-align="center" data-formatter="paidAmountFormatter">{{ __('paid_amount') }}</th>
                                    <th scope="col" data-field="operate" data-sortable="false" data-events="feesPaidEvents" data-formatter="actionFormatter" data-align="center" data-escape="false"> {{ __('Action') }}</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
...
@section('js')
    <script>
        ...
        function discountFormatter(value, row) {
            ...
            if (!row) {
                return '<span class="badge badge-secondary" style="font-size: 12px;">-</span>';
            }
            
            // Check if discount exists and is greater than 0
            let discountAmount = value || row.discount_amount || 0;
            
            if (discountAmount && parseFloat(discountAmount) > 0) {
                return '<span class="badge badge-success" style="font-size: 13px; padding: 6px 12px;">' +
                       '<i class="mdi mdi-sale"></i> ' + parseFloat(discountAmount).toFixed(2) +
                       '</span>';
            }
            return '<span class="badge badge-secondary" style="font-size: 12px; padding: 4px 8px;">-</span>';
        }

        // Paid Amount Formatter - Shows the actual paid amount with discount applied
        function paidAmountFormatter(value, row) {
            // Handle undefined or null values
            if (!row) {
                return '0.00';
            }
            
            // Get compulsory fees total from fees table (base amount)
            let compulsoryFeesTotal = parseFloat(row.fees?.total_compulsory_fees || 0);
            
            // Get discount amount to subtract from base amount
            let discountAmount = parseFloat(row.discount_amount || 0);
            
            // Calculate base amount after discount
            let baseAmount = compulsoryFeesTotal - discountAmount;
            
            // Add due charges if they exist (for late installment payments)
            let dueCharges = parseFloat(row.compulsory_fees_sum_due_charges || 0);
            
            // Add extra fees if they exist
            let extraFee = parseFloat(row.compulsory_fees_sum_extra_fee || 0);
            
            // Calculate total paid amount: (Compulsory Fees - Discount) + Due Charges + Extra Fee
            let totalPaid = baseAmount + dueCharges + extraFee;
            
            // Format with 2 decimal places
            if (discountAmount > 0 || dueCharges > 0 || extraFee > 0) {
                // Build breakdown string
                let breakdown = '<br><small class="text-muted">(';
                
                if (discountAmount > 0) {
                    breakdown += compulsoryFeesTotal.toFixed(2) + ' - Disc: ' + discountAmount.toFixed(2);
                } else {
                    breakdown += 'Amt: ' + compulsoryFeesTotal.toFixed(2);
                }
                
                if (dueCharges > 0) {
                    breakdown += ' + Due: ' + dueCharges.toFixed(2);
                }
                
                if (extraFee > 0) {
                    breakdown += ' + Extra: ' + extraFee.toFixed(2);
                }
                
                breakdown += ')</small>';
                
                return '<span style="font-weight: 500; color: #28a745;">' + totalPaid.toFixed(2) + '</span>' + breakdown;
            }
            
            return '<span style="font-weight: 500; color: #28a745;">' + totalPaid.toFixed(2) + '</span>';
        }

        // Action Formatter with Print Button
        function actionFormatter(value, row) {
            // Check if required data exists
            if (!row || !row.fees || !row.student_id) {
                console.error('Missing required data in row:', row);
                return value || ''; // Return original value if data is missing
            }
            
            let buttons = '';
            let feesId = row.fees.id || row.fees_id || '';
            let studentId = row.student_id || row.student?.id || '';
            
            // Pay Button (Green) - For unpaid or partial paid
            if (row.fees_status !== 1 && feesId && studentId) {
                buttons += '<a href="' + baseUrl + '/fees/compulsory/' + feesId + '/' + studentId + '" ' +
            ...
            // Receipt Button (Blue) - For paid or partial paid
            if ((row.fees_status === 1 || row.fees_status === 2) && feesId && studentId) {
                buttons += '<a href="' + baseUrl + '/fees/compulsory-fees-receipt/' + studentId + '/' + feesId + '" ' +
                          'class="btn btn-xs btn-gradient-primary btn-rounded btn-icon" ' +
                          'target="_blank" title="View Receipt">' +
                          '<i class="mdi mdi-file-document"></i>' +
                          '</a> ';
            }
            
            // Print Button (Orange) - For paid or partial paid
            if ((row.fees_status === 1 || row.fees_status === 2) && feesId && studentId) {
                buttons += '<a href="' + baseUrl + '/fees/compulsory-fees-receipt/' + studentId + '/' + feesId + '?print=1" ' +
                          'class="btn btn-xs btn-gradient-warning btn-rounded btn-icon print-receipt-btn" ' +
