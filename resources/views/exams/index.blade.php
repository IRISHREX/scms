...
@section('content')
    <div class="content-wrapper">
        ...
        'required' => true,
                                ...
                                <div class="form-group col-sm-12 col-md-12 col-lg-4">
                                    <label for="session_year_id">{{ __('Session Years') }} <span
                                ...
                                <div class="form-group col-sm-12 col-md-12 col-lg-4">
                                    <label>{{ __('Marks Distribution') }} <span class="text-danger">*</span></label>
                                    <div id="exam-type-container">
                                        <div class="input-group mb-3">
                                            <input type="text" name="exam_type[]" class="form-control"
                                                placeholder="Enter type e.g. Oral, Written, Practical">
                                            <div class="input-group-append">
                                                <button class="btn btn-danger remove-exam-type" type="button"
                                                    style="display:none;"><i class="fa fa-times"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                    <button type="button" class="btn btn-sm btn-gradient-primary"
                                        id="add-exam-type">{{ __('Add New') }}</button>
                                </div>
                            </div>
                            <div class="row">
                        ...
                        <table aria-describedby="mydesc" class='table' id='table_list' data-toggle="table"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" data-field="exam_type" data-formatter="examTypeFormatter">
                                        {{ __('Exam Type') }}
                                    </th>
        ...
        'rows' => '3',
                                    ...
                                    <div class="form-group col-12">
                                        <label>{{ __('Marks Distribution') }} <span class="text-danger">*</span></label>
                                        <div id="edit-exam-type-container">

                                        </div>
                                        <button type="button" class="btn btn-sm btn-gradient-primary"
                                            id="edit-add-exam-type">{{ __('Add New') }}</button>
                                    </div>
                                </div>
                            </div>
...
@section('script')
    <script>
        $(document).ready(function () {
            $('#add-exam-type').click(function () {
                $('#exam-type-container').append(`
                    <div class="input-group mb-3">
                        <input type="text" name="exam_type[]" class="form-control" placeholder="Enter mark distribution type" required>
                        <div class="input-group-append">
                            <button class="btn btn-danger remove-exam-type" type="button"><i class="fa fa-times"></i></button>
                        </div>
                    </div>
                `);
            });

            $('#edit-add-exam-type').click(function () {
                $('#edit-exam-type-container').append(`
                    <div class="input-group mb-3">
                        <input type="text" name="exam_type[]" class="form-control" placeholder="Enter mark distribution type" required>
                        <div class="input-group-append">
                            <button class="btn btn-danger remove-exam-type" type="button"><i class="fa fa-times"></i></button>
            ...
            $(document).on('click', '.remove-exam-type', function () {
        ...
        const formSuccessFunction = () => {
            setTimeout(() => {
                ...
                $('#exam-type-container').find('.input-group:not(:first)').remove();
                $('#exam-type-container').find('input').val('');
            }, 500);
        }

        function examTypeFormatter(value, row) {
            if (row.exam_type && Array.isArray(row.exam_type)) {
                return row.exam_type.join(', ');
            }
            return '';
        }
        ...
        window.examEvents = {
            'click .edit-data': function (e, value, row, index) {
                ...
                $('#edit-exam-type-container').empty();
                if (row.exam_type) {
                    $.each(row.exam_type, function (index, item) {
                        $('#edit-exam-type-container').append(`
                                <div class="input-group mb-3">
                                    <input type="text" name="exam_type[]" class="form-control" placeholder="Enter mark distribution type" value="${item}" required>
                                    <div class="input-group-append">
                                        <button class="btn btn-danger remove-exam-type" type="button"><i class="fa fa-times"></i></button>
                                    </div>
                                </div>
        ...
        $(document).on('click', '.publish-exam-result', function (e) {
            ...
            if (!confirm('{{ __('Are you sure you want to change publish status?') }}')) {
                return;
            }

            var url = '{{ url('exams/publish') }}' + '/' + id;
            ...
            $.ajax({
                ...
                success: function (res) {
                    // refresh table to get updated publish status
